...
import static net.biryeongtrain06.stat_system.util.setItemStat.ItemStatKeys.ITEM_STRENGTH_KEY;

public class StrengthRegistry extends StatRegistry{
    @Override
    public NbtCompound setStat(NbtCompound nbt, int level, int rarity) {
        ...
        result.putInt(ITEM_STRENGTH_KEY, MathHelper.ceil(((Math.random() * maxValue)) + minValue));
        return result;
    }
}
