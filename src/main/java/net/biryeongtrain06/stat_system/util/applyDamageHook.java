...
public class applyDamageHook {
        public static void hook (Entity target, Entity source, DamageSource dmgSource, float damage) {
            if (target instanceof PlayerEntity) {

            }
            else if (target instanceof Entity && dmgSource.getAttacker().isPlayer()) {
                onPlayerDamageToEntity.applyDamage(target, source, dmgSource, damage);
            }
        }
